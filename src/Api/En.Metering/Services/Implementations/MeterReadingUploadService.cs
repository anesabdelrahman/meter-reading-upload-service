        ...
        public async Task<(int success, int failed)> ProcessCsvAsync(Stream csvStream, CancellationToken token = default)
        {
            ...
            try
            {
                await foreach (var record in _csvParser.ParseCsvAsync(csvStream, token))
                {
                    ...
                    var capturedRecord = record; // prevent closure reuse
                    ...
                    var task = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            // Do NOT use parent token here (it cancels all workers)
                            var result = await processor.ProcessRecordAsync(capturedRecord, CancellationToken.None);
